function datePrice(option) {
    ...
    if(currentMonth+monthNum<12){
        initDom(currentYear,yf,monthNum,allData);
    }else{
        initDom(currentYear,yf,12-parseInt(currentMonth),allData);
...
var initDom=function (startY,startM,num,allDa) {
    var DaysInMonth = [];
    var html="";
        ...
        for (var i = 0; i < Ntr; i++) {
            ...
            for (var j = 0; j < 7; j++) {
                ...
                if((i==0&&j<firstDay)||(i==(Ntr-1)&&x-firstDay>=DaysInMonth[q-1])){
                    ...
                    var md=m++;
                    if(flag==1&&md<new Date(startY).getDate()){
